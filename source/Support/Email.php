...
class Email
{
     ...
     * Email constructor.
     */
    public function __construct(string $servermail = 'sendgrid')
    {
    ...
    /*
        Enviar pelo Servidor alternativo caso o SendGrid esteja em blacklista para alguns provedores
     */
    public function authProvider():bool
    {
     ...
     * @param int $perSecond
     */
    public function sendQueue(int $perSecond = 1, bool $blacklist = false)
    {
      
    $stmt = Connect::getInstance()->query("SELECT * FROM mail_queue WHERE sent_at IS NULL");
        
    //var_dump(CONF_MAIL_PASS,$this->mail->Password);
        $count = 0;
        if ($stmt->rowCount()) {
